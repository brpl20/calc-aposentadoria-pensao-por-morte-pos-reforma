...
function calcFunction(min_ano, user_value) {
	...
	if (sobra <= 1) {
		...
		logs += `<br/>Valor Do Mínimo R$: ${min_ano}`;
		logs += `<br/>Valor da Primeira Sobra (60%) R$: ${(1*min_ano)*0.6}`;
		logs += `<br/>Valor da Segunda Sobra (40%) R$: ${((sobra-1)*min_ano)*0.4}`;
		return [(((1*min_ano)*0.6) + min_ano + (((sobra-1)*min_ano)*0.4)), logs];
	} else if (sobra > 2 && sobra <= 3) {
		logs += `<br/>Valor Do Mínimo R$: ${min_ano}`;
		logs += `<br/>Valor da Primeira Sobra (60%): ${(1*min_ano)*0.6}`;
		logs += `<br/>Valor da Segunda Sobra (40%): ${(1*min_ano)*0.4}`;
		logs += `<br/>Valor da Terceira Sobra (20%): ${((sobra-2)*min_ano)*0.2}`;
		...
		logs += `<br/>Valor da Primeira Sobra (60%): ${(1*min_ano)*0.6}`;
		logs += `<br/>Valor da Segunda Sobra (40%): ${(1*min_ano)*0.4}`;
		logs += `<br/>Valor da Terceira Sobra (20%): ${(1*min_ano)*0.2}`;
